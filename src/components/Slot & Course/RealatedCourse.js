...
import { Card, Row, Col, Spinner, NavDropdown } from "react-bootstrap";
...
const RelatedCourse = () => {
  ...
  const handleRequest = async (e, subjectId, lecturerId) => {
    ...
    isLoading(true);
    await axios
      .post(`/api/v1/requests/student/${loginUser.userId}`, {
        ...
        related.map((course, i) => (
          ...
          >
            <div className={`${Style.cardContainer}`}>
              <Card className={`${Style.cardSendRequest}`}>
                <Card.Body className="border-top">
                  ...
                  <form
                    onSubmit={(e) =>
                      handleRequest(e, course.subjectId, course.lecturerId)
                    }
                  >
                    ...
                    <button>
                      <NavDropdown.Item
                        href={`/student/lecturer/profile/${course.lecturerId}`}
                      >
                        View profile
                      </NavDropdown.Item>
                    </button>
